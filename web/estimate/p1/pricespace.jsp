...
<%
	...
	try
	{
		...
		Data limits = datasource.find("select MODEL, PRICE, max(VOLUME) as MAX, min(VOLUME) as MIN from T_TOTAL_SALES where CREATE_USER_ID = ? and (year = ? or year = ? or year = ?) group by MODEL", usercode, String.valueOf(year - 1), String.valueOf(year - 2), String.valueOf(year - 3));
		...
		for(Datum item : items)
		{
			...
			//element.put("volumelower", Math.floor( item.getDouble("NUM") * 100 ));
			//element.put("volumeupper", Math.ceil( item.getDouble("NUM") * 100 ));
			//element.put("volumelower", item.getDouble("NUM") * 100);
			//element.put("volumeupper", item.getDouble("NUM") * 100);
			element.put("volumelower", Math.floor(item.getDouble("GROUPMIN") / totalmin * 100 ));
			element.put("volumeupper", Math.ceil(item.getDouble("GROUPMAX") / totalmax * 100 ));
			array.put(element);
		}
