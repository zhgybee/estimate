...
<%
	...
	for(int j = 0 ; j < rows.length() ; j++)
	{
		...
		sqls.add("insert into T_SELECTED_PLAN(ID, YEAR, GROUPNAME, GROUPVOLUME, BRAND, MODEL, MAX, MIN, REMAX, REMIN, PRICE, COST, PROFIT, X, ISINSIDE, CREATE_USER_ID, CREATE_DATE) values('"+
		SystemUtils.uuid()+"', '"+year+"', '"+row.optString("GROUPNAME")+"', '"+row.optString("GROUPVOLUME")+"', '"+row.optString("BRAND")+"', '"+row.optString("MODEL")+"', '"+row.optString("MAX")+"', '"+row.optString("MIN")+"', '"+row.optString("REMAX")+"', '"+row.optString("REMIN")+"', '"+row.optString("PRICE")+"', '"+row.optString("COST")+"', '"+row.optString("PROFIT")+"', '"+row.optString("X")+"', '"+row.optString("ISINSIDE")+"', '"+usercode+"', '"+SystemUtils.toString(Calendar.getInstance())+"')");
	}
